...
def define_shikra_vms(vm_image_opts = vm_image_opts()):
    base_target = "shikra-tuivm"
    for variant in vm_variants:
        base_tv = "{}_{}".format(base_target, variant)

        dtb_list = get_dtb_list(base_target)
        compiled_dtbs = ["//soc-repo:shikra-{}_{}_dtb_build/{}".format(vt, variant, t) for vt in vm_types for t in dtb_list]
        ...
        out_dtb_list = [":shikra-{}_{}_vm_dtb_img".format(vt, variant) for vt in vm_types]
        dist_targets = (
            ["shikra-{}_{}_vm_dist".format(vt, variant) for vt in vm_types] +
            ["shikra-{}_{}_dist".format(vt, variant) for vt in vm_types] +
            [":shikra-{}_{}_modules_install".format(vt, variant) for vt in vm_types] +
            [":shikra-{}_{}_signed_modules".format(vt, variant) for vt in vm_types] +
            ["shikra-{}_{}_merge_msm_uapi_headers".format(vt, variant) for vt in vm_types] +
            [base_kernel] +
            [":signing_key"] +
            [":verity_key"] +
