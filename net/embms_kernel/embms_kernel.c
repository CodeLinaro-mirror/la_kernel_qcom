 ...
 * tunneling, net_device and tunneling related
 * structs and params
 */

static unsigned char hdr_buff[sizeof(struct iphdr) + sizeof(struct udphdr)];
...
static int handle_multicast_stream(struct sk_buff *skb)
{
	...
	if (embms_conf.embms_tunneling_status == TUNNELING_OFF) {
		DEBUGP("Tunneling Disabled. Can't process packets\n");
		return 0;
	}

	if (unlikely(memcmp(skb->dev->name, embms_conf.embms_iface,
			    strlen(embms_conf.embms_iface)) != 0)) {
		pr_err("Packet received on %s iface. NOT an EMBMS Iface\n",
		       skb->dev->name);
		return 0;
	}
	...
	if (tmgi_entry_ptr == &tmgi_to_clnt_map_tbl.tmgi_list_ptr) {
		pr_err("could not find matchin tmgi entry\n");
	...
	if (unlikely(!skb_new)) {
		pr_err("Can't allocate headroom\n");
		spin_unlock_bh(&embms_conf.lock);
		return 0;
	}
...
static int embms_device_open(struct inode *inode, struct file *file)
{
	...
	if (check_embms_device(&embms_conf.device_under_use)) {
		pr_err("embms_tm_open : EMBMS device busy\n");
		return -EBUSY;
	}
					...
					struct tmgi_to_clnt_info_update *clnt)
{
	...
	DEBUGP("check_for_client_entry: clnt addr :%pI4, port %u\n",
			      ...
			      struct clnt_info *clnt)
{
	...
	DEBUGP("New tmgi multicast addr :%pI4 , port %u\n",
	       &info_update->multicast_addr,
	       ntohs(info_update->multicast_port));

	DEBUGP("Adding client entry\n");
...
static void print_tmgi_to_client_table(void)
{
	...
	DEBUGP("====================================================\n");
	DEBUGP("Printing TMGI to Client Table :\n");
	DEBUGP("No of Active TMGIs : %d\n", embms_conf.no_of_tmgi_sessions);
	DEBUGP("====================================================\n\n");
	...
	if (embms_conf.no_of_tmgi_sessions > 0) {
		...
		list_for_each_safe(tmgi_entry_ptr, prev_tmgi_entry_ptr,
			...
			list_for_each_safe(clnt_ptr, prev_clnt_ptr,
				...
				temp_client = list_entry(clnt_ptr,
							 struct clnt_info,
							 client_list_ptr);
				DEBUGP("Client entry %d :\n", j);
				DEBUGP("client addr : %pI4 , port %u\n\n",
		...
		DEBUGP("No TMGI entries to Display\n");
	}
	DEBUGP("==================================================================\n\n");
}
...
int delete_tmgi_entry_from_table(char *buffer)
{
	 ...
	 */
	if (embms_conf.no_of_tmgi_sessions == 0) {
		pr_err("TMGI count 0. Nothing to delete\n");
		return SUCCESS;
	}
	...
	if (!temp_tmgi) {
		...
		pr_err("Desired TMGI entry not found\n");
		return -EINVAL;
	}
	...
	list_for_each_safe(clnt_ptr, prev_clnt_ptr,
		...
		temp_client = list_entry(clnt_ptr,
					 struct clnt_info,
					 client_list_ptr);
		DEBUGP("Client addr to delete :%pI4 , port %u\n",
		       &temp_client->addr, ntohs(temp_client->port));
...
int delete_client_entry_from_all_tmgi(char *buffer)
{
	...
	DEBUGP("del_clnt_from_all_tmgi: Enter\n");

	info_update = (struct tmgi_to_clnt_info_update *)buffer;

	if (!info_update) {
		pr_err("del_clnt_from_all_tmgi: NULL arguments passed\n");
		return -EINVAL;
	}
	...
	list_for_each_safe(tmgi_entry_ptr, prev_tmgi_entry_ptr,
		...
		if (tmgi->no_of_clients == 0) {
			 ...
			 * entry from table
			 */
			DEBUGP("del_clnt_from_all_tmgi:");
			DEBUGP("Deleted client was ");
			DEBUGP("last client for tmgi\n");
			DEBUGP("del_clnt_from_all_tmgi:");
			DEBUGP("Delting tmgi as it has ");
			DEBUGP("zero clients.TMGI IP ");
			DEBUGP(":%pI4 , port %u\n",
			       &tmgi->tmgi_multicast_addr, ntohs(tmgi->tmgi_port));
	...
	DEBUGP("del_clnt_from_all_tmgi Successful\n");
	return SUCCESS;
}
...
int add_client_entry_to_table(char *buffer)
{
	...
	neigh_entry = __ipv4_neigh_lookup(dev_global,
					  (u32)(new_client->addr));
	if (!neigh_entry) {
		pr_err("Can't find neighbour entry\n");
		kfree(new_client);
		return -EINVAL;
	}
	...
	DEBUGP("DMAC of client : %pM\n", new_client->dmac);
	DEBUGP("New client addr :%pI4 , port %u\n",
	...
	if (embms_conf.no_of_tmgi_sessions == 0) {
		...
		DEBUGP("tmgi_to_clnt_map_tbl is empty\n");
	...
	if (tmgi) {
		...
		DEBUGP("TMGI entry not present. Adding tmgi entry\n");
...
int delete_client_entry_from_table(char *buffer)
{
	...
	if (!temp_tmgi) {
		pr_err("TMGI not found\n");
		return -EINVAL;
	}
	/* Delete client entry for a specific tmgi*/

	DEBUGP("clnt addr :%pI4,port %u\n",
	       &info_update->client_addr,
	       ntohs(info_update->client_port));
	...
	DEBUGP("Client entry deleted\n");

	if (temp_tmgi->no_of_clients == 0) {
		/* If deleted client was the only client for that TMGI
		 * we need to delete TMGI entry from table
		 */
		DEBUGP("Deleted client was the last client for tmgi\n");
		DEBUGP("Deleting tmgi since it has zero clients\n");
			...
			unsigned long ioctl_param)
{
	...
	case GET_EMBMS_TUNNELING_STATUS:
		 ...
		 */
		DEBUGP("Sending tunneling status : %d\n",
	...
	case START_EMBMS_TUNNEL:
		...
		if (!dev_global) {
			...
			if (iface_info) {
				DEBUGP("IP address of %s iface is %pI4\n",
				       BRIDGE_IFACE,
				...
				DEBUGP("Could not find iface address\n");
				ret = FAILURE;
			}
		}
...
static int embms_ioctl_init(void)
{
	...
	if (ret) {
		pr_err("device_alloc err ret:%d\n", ret);
		goto dev_alloc_err;
	}

	embms_class = class_create(EMBMS_DEVICE_NAME);
...
static int __init start_embms(void)
{
	...
	DEBUGP("Registering embms device\n");

	ret = embms_ioctl_init();
	if (ret) {
		pr_err("embms device failed to register\n");
		goto fail_init;
	}
	...
	udph_global->check = EMBMS_UDP_CHECKSUM;
	iph_global->version = EMBMS_IP_VERSION;
	iph_global->ihl = EMBMS_IP_IHL;
	iph_global->tos = EMBMS_IP_TOS;
	iph_global->frag_off = EMBMS_IP_FRAG_OFFSET;
	iph_global->ttl = EMBMS_IP_TTL;
