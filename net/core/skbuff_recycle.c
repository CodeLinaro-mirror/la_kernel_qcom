...
do { \
	typeof(skb) _skb = (skb); \
	kmemleak_update_trace(_skb);\
	kmemleak_ignore(_skb);\
	kmemleak_update_trace(_skb->head);\
	kmemleak_ignore(_skb->head);\
} while (0)
...
#endif
	cpuhp_setup_state_nocalls(CPUHP_SKB_RECYCLER_DEAD,
				  "net/skbuff_recycler:dead:", NULL, skb_cpu_callback);
...
EXPORT_SYMBOL_GPL(check_skb_fast_recyclable);
#endif
