...
#define SKB_DATA_CACHE_SIZE ( \
		SKB_DATA_ALIGN(SKB_RECYCLE_SIZE + NET_SKB_PAD) +\
		SKB_DATA_ALIGN(sizeof(struct skb_shared_info)) \
		)
#else
...
#else
#define SKB_DATA_CACHE_SIZE ( \
			(SKB_DATA_ALIGN(1856 + NET_SKB_PAD)) + \
			SKB_DATA_ALIGN(sizeof(struct skb_shared_info)) \
			)
#endif
#endif
			     ...
			     bool *pfmemalloc)
{
	...
	if ((obj_size <= SKB_SMALL_HEAD_CACHE_SIZE &&
		...
		skb_cache = (obj_size <= SKB_SMALL_HEAD_CACHE_SIZE)
			? skb_small_head_cache
			: skb_data_cache;
		obj = kmem_cache_alloc_node(skb_cache,
					    flags | __GFP_NOMEMALLOC | __GFP_NOWARN,
					    node);
		*size = (obj_size <= SKB_SMALL_HEAD_CACHE_SIZE)
...
EXPORT_SYMBOL_GPL(__netdev_alloc_skb_fast);
...
EXPORT_SYMBOL_GPL(consume_skb_list_fast);
