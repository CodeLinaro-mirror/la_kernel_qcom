		...
		u8 protocol)
{
	...
	if (protocol == IPPROTO_IPIP) {
		...
		for (fmr = t->parms.fmrs; fmr; fmr = fmr->next) {
			unsigned int mshift = 32 - fmr->ip4_prefix_len;
				  ...
				  struct __ip6_tnl_parm *parms)
{
	...
	if (data[IFLA_IPTUN_FMRS]) {
		...
		nla_for_each_nested(fmr, data[IFLA_IPTUN_FMRS], rem) {
			...
			c = fmrd[IFLA_IPTUN_FMR_IP6_PREFIX];
			if (c)
				nla_memcpy(&nfmr->ip6_prefix, fmrd[IFLA_IPTUN_FMR_IP6_PREFIX],
					   sizeof(nfmr->ip6_prefix));

			c = fmrd[IFLA_IPTUN_FMR_IP4_PREFIX];
			if (c)
				nla_memcpy(&nfmr->ip4_prefix, fmrd[IFLA_IPTUN_FMR_IP4_PREFIX],
					   sizeof(nfmr->ip4_prefix));

			c = fmrd[IFLA_IPTUN_FMR_IP6_PREFIX_LEN];
			if (c)
				nfmr->ip6_prefix_len = nla_get_u8(c);

			c = fmrd[IFLA_IPTUN_FMR_IP4_PREFIX_LEN];
			if (c)
				nfmr->ip4_prefix_len = nla_get_u8(c);

			c = fmrd[IFLA_IPTUN_FMR_EA_LEN];
			if (c)
				nfmr->ea_len = nla_get_u8(c);

			c = fmrd[IFLA_IPTUN_FMR_OFFSET];
			if (c)
...
EXPORT_SYMBOL_GPL(ip6_tunnel_is_fallback_dev);
