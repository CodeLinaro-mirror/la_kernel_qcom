...
/* Number of microseconds represented by bucket 0. */
#define NETDEV_SAWF_HIST_BASE_US	1
...
struct pcpu_sawf_stats {
	...
	/* Delay histogram; 8 bins over 256 potential service classes. */
	u64	delay[NETDEV_SAWF_SID_MAX][NETDEV_SAWF_DELAY_BUCKETS];
	...
	/* Minimum measured latency for specified debug service class. */
	u32	debug_lat_min;
	/* Exponential weighted moving average latency for specified debug service class. */
	u32	debug_lat_ewma;
...
bool netdev_sawf_debug_get(struct net_device *dev, uint8_t *sid,
			   uint32_t *max, uint32_t *min, uint32_t *avg, uint32_t *last);
...
{
	return (dev->sawf_flags & NETDEV_SAWF_FLAG_DEBUG) &&
		((dev->sawf_flags >> NETDEV_SAWF_FLAG_DEBUG_SHIFT) == sid);
}
...
static inline void netdev_sawf_latency_record(struct sk_buff *skb, struct net_device *dev)
{
	 ...
	 */
	if ((dev->sawf_flags &
	    (NETDEV_SAWF_FLAG_ENABLED | NETDEV_SAWF_FLAG_TX_LAT)) != NETDEV_SAWF_FLAG_ENABLED)
		return;
	...
	sawf_stats->debug_lat_ewma = sawf_stats->debug_lat_ewma -
				     (sawf_stats->debug_lat_ewma >> 8) + (lat >> 8);
