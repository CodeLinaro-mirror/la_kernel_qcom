...
/**
 * geni_se_read_vote: Function to read vote values set using dt property.
 * @wrapper_node: wrapper device node.
 * @path: path value describing geni to core, cpu to geni, or geni to ddr.
 * @vote_index_value: index values for the respective path.
 * @dev: Associated device.
 *
 * This function reads vote values corresponding to the path.
 *
					       ...
					       struct geni_se *se_rsc)
{
	...
	int len, i, no_of_entries;
	u32 geni_to_core, cpu_to_geni, geni_to_ddr;
	...
	if (!vote_index_list || len % sizeof(u32)) {
		dev_err(&pdev->dev, "Property %s not found or invalid, len: %d\n",
			VOTE_INDEX_PROP_NAME, len);
		goto dts_err;
	}
	...
	return geni_se_common_resources_init(se_rsc, geni_to_core, cpu_to_geni, geni_to_ddr);
