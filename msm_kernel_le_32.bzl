...
EOF
    """ % (
        " ".join([v.replace("-", "_") for v in le_32_variants]),  # VARIANTS
...
def define_msm_le_32(
    ...
    if not variant in le_32_variants:
        fail("{} not defined in target_variants.bzl target_variants!".format(variant))
    ...
    dtstree = dtstree.replace("arm64", "arm")

    vendor_ramdisk_binaries = get_vendor_ramdisk_binaries(target, flavor = "le-32")
