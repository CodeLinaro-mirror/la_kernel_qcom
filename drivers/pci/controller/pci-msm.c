...
/*
 * Program the PWR_CTRL/PWR_MASK override-enable for one PCIe instance.
 * Shared by the boot-time programming in pcie_init() and the post-hibernation
 * restore in msm_pcie_syscore_resume() so both paths use identical addressing.
 */
static void msm_pcie_program_sm_override(void __iomem *base, int inst, u32 val)
{
	u32 stride = inst * pcie_sm_regs[PCIE_SM_PWR_INSTANCE_OFFSET];

	msm_pcie_write_reg(base, pcie_sm_regs[PCIE_SM_PWR_CTRL_OFFSET] + stride, val);
	msm_pcie_write_reg(base, pcie_sm_regs[PCIE_SM_PWR_MASK_OFFSET] + stride, val);
}
 ...
 * are reset when the register space is reset during hibernation.
 *
...
static void msm_pcie_syscore_resume(void)
{
	...
	for (i = 0; i < pcie_sm_regs[PCIE_SM_NUM_INSTANCES]; i++)
		msm_pcie_program_sm_override(pcie_sm_base, i, 0x1);
...
static int __init pcie_init(void)
{
	...
	if (count == MAX_PCIE_SM_REGS &&
	    pcie_sm_regs[PCIE_SM_NUM_INSTANCES] > 0 &&
		 ...
		 */
		size_t map_size = pcie_sm_regs[PCIE_SM_PWR_MASK_OFFSET] +
