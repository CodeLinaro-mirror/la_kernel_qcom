...
#else
#define	TSENS_DBG1(dev, msg, x...) do {					\
		(void)(dev);		\
		pr_debug(msg, ##x);	\
	} while (0)
#define	TSENS_DBG(dev, msg, x...) do {					\
		(void)(dev);		\
		pr_debug(msg, ##x);	\
	} while (0)
#define	TSENS_INFO(dev, msg, x...) do {					\
		(void)(dev);		\
		pr_info(msg, ##x);	\
	} while (0)
#define	TSENS_ERR(dev, msg, x...) do {					\
		(void)(dev);		\
		pr_err(msg, ##x);	\
	} while (0)
#define	TSENS_DUMP(dev, msg, x...) do {					\
		(void)(dev);		\
		pr_info(msg, ##x);	\
	} while (0)
#endif
...
extern const struct tsens_data data_tsens14xx, data_tsens14xx_9607;
