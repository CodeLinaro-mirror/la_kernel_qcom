				...
				struct arm_smmu_domain *old_domain)
{
	...
	for (i = 0; i < master->num_streams; ++i) {
		...
		ret = arm_vsmmu_impl_attach_table(smmu, new_domain, old_domain, step->data, sid);
		if (ret)
			break;
	}
	...
	if (ret) {
		for (i--; i >= 0; i--) {
			...
			arm_vsmmu_impl_attach_table(smmu, NULL, new_domain, step->data, sid);
		}
	}
 ...
 */
static void arm_vsmmu_impl_sync_cd(struct arm_smmu_master *master,
				int ssid, bool leaf)
{
	...
	req.domain = cpu_to_le32(master->domain->virtio.id);
	req.pasid = cpu_to_le32(ssid);

	arm_vsmmu_impl_invalidate(master->domain, &req);
}
