					...
					struct spi_controller *spi)
{
	...
	if (mode_changed & SPI_LSB_FIRST)
		/* force next setup_se_xfer to call spi_setup_word_len */
		mas->cur_bits_per_word = 0;
	if (mode_changed & SPI_LOOP)
		writel((spi_slv->mode & SPI_LOOP)
	...
	if ((mode_changed & SPI_CS_HIGH) || (cs_changed && (spi_slv->mode & SPI_CS_HIGH)))
		writel((spi_slv->mode & SPI_CS_HIGH)
		       ? BIT(chipselect) : 0, se->base + SE_SPI_DEMUX_OUTPUT_INV);
