...
static void _destroy_boot_marker(const char *name)
{
	...
	list_for_each_entry_safe(marker, temp_addr, &boot_marker_list.list, list) {
...
EXPORT_SYMBOL_GPL(update_marker);
...
static int mpm_parse_dt(void)
{
	...
	np_mpm2 = of_find_compatible_node(NULL, NULL, "qcom,mpm2-sleep-counter");
	if (!np_mpm2) {
	...
	np_mpm_hr = of_find_compatible_node(NULL, NULL, "qcom,mpm-hr-counter");
	if (!np_mpm_hr) {
	...
	if (!mpm_hr_counter_base) {
		pr_err("mpm_hr_counter: can't map counter base\n");
