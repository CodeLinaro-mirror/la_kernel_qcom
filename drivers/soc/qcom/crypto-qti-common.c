...
struct ice_part_cfg {
	...
	bool decr_bypass;
	struct kobject kobj;
	const struct kobj_type *kobj_type;
};
...
struct ice_device {
	...
	const struct kobj_type	*partitions_kobj_type;
	u32			num_fde_slots;
...
ATTRIBUTE_GROUPS(ice_part_cfg);

static const struct kobj_type crypto_qti_ice_kobj_type = {
	.release        = crypto_qti_ice_release_kobj,
	.sysfs_ops      = &kobj_sysfs_ops,
	.default_groups = ice_part_cfg_groups,
};
					    ...
					    bool new_key)
{
	...
	elem->kobj_type = &crypto_qti_ice_kobj_type;
	...
	if (ice_dev->num_fde_slots_in_use >= TOTAL_NUMBER_ICE_SLOTS) {
		...
		rc = -ENOMEM;
		dev_err(ice_dev->pdev, "%s: Allocating more slots than available\n", __func__);
		kobject_put(&elem->kobj);
		kfree(elem);
		goto out;
	}
				   ...
				   size_t count)
{
	...
	if (count > PARTITION_META_INFO_VOLNAMELTH) {
		dev_err(dev, "Invalid partition '%s' (%zu)\n", buf, count);
		return -EINVAL;
	}
...
static const struct kobj_type crypto_qti_ice_partitions_kobj_type = {
	.release        = crypto_qti_ice_release_kobj,
	.sysfs_ops      = &kobj_sysfs_ops,
	.default_groups = ice_groups,
};
...
EXPORT_SYMBOL_GPL(crypto_qti_ice_add_userdata);
...
int crypto_qti_ice_init_fde_node(struct device *dev)
{
	...
	ice_dev->partitions_kobj_type = &crypto_qti_ice_partitions_kobj_type;
