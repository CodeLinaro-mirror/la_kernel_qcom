...
#define XVM_SCHE_BUF_SIZE 2U

static unsigned long long xvm_sche_tx_tv_buffer[XVM_SCHE_BUF_SIZE];

static void pipe_read_trace(struct qvm_channel *dev,
			int size, int ret)
{
	const struct hab_pipe_endpoint *ep = dev->pipe_ep;
	const struct hab_shared_buf *sh_buf = dev->rx_buf;
	...
	it->sz = (unsigned int)size;
	it->ret = (unsigned int)ret;
		...
		size_t read_size)
{
	...
	if (dev != NULL) {
		...
		/* log */
		pipe_read_trace(dev, (int)read_size, ret);
		...
		unsigned int flags)
{
	...
	struct qvm_channel *dev  = (struct qvm_channel *)pchan->hyp_data;
	size_t payload_size = sizeof(*header) + sizebytes;
	const uint32_t buf_size = PIPE_SHMEM_SIZE;
	const int disabled_irqs = irqs_disabled();

	/* Only used in virtio arch */
	(void)flags;

	if (payload_size > (size_t)buf_size)
	...
	if ((buf_size -
		(uint32_t)(dev->pipe_ep->tx_info.wr_count -
		dev->tx_buf->rd_count)) < (uint32_t)payload_size) {
	...
	header->sequence = pchan->sequence_tx + 1U;
	...
	if (HAB_HEADER_GET_TYPE(*header) == (uint32_t)HAB_PAYLOAD_TYPE_PROFILE) {
		...
		if (pstat != NULL) {
			...
			pstat->tx_usec = (uint32_t)((uint64_t)ts.tv_nsec / (uint64_t)NSEC_PER_USEC);
		} else {
			hab_spin_unlock(&dev->io_lock, disabled_irqs);
		...
		== (uint32_t)HAB_PAYLOAD_TYPE_SCHE_RESULT_REQ) {
		((unsigned long long *)payload)[0] = xvm_sche_tx_tv_buffer[0];
	} else if (HAB_HEADER_GET_TYPE(*header)
		== (uint32_t)HAB_PAYLOAD_TYPE_SCHE_RESULT_RSP) {
	...
	hab_spin_unlock(&dev->io_lock, disabled_irqs);
	if (HAB_HEADER_GET_TYPE(*header) == (uint32_t)HAB_PAYLOAD_TYPE_SCHE_MSG)
		xvm_sche_tx_tv_buffer[0] = msm_timer_get_sclk_ticks();
	else if (HAB_HEADER_GET_TYPE(*header) == (uint32_t)HAB_PAYLOAD_TYPE_SCHE_MSG_ACK)
...
void shmem_physical_channel_rx_dispatch(unsigned long data)
{
	...
	while (1) {
		...
		/* debug */
		pipe_read_trace(dev, (int)sizeof(header), ret);

		if ((uint32_t)ret == 0xFFFFFFFFU) { /* signature mismatched first time */
		...
		} else if ((uint32_t)ret == 0xFFFFFFFEU) { /* continuous signature mismatches */
			continue;
		...
		if ((pchan->sequence_rx + 1U) != header.sequence) {
		...
		(void)hab_msg_recv(pchan, &header);
	}
	hab_spin_unlock(&pchan->rxbuf_lock, disabled_irqs);
}
