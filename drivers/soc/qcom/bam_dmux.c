...
/* Alignment requirements for BAM DMUX packets */
#define BAM_DMUX_ALIGNMENT		4
#define BAM_DMUX_ALIGN_MASK		(BAM_DMUX_ALIGNMENT - 1)
...
#ifdef CONFIG_MSM_BAM_DMUX_DEBUG
...
static void bam_mux_write_done(struct work_struct *work)
{
	...
	if (unlikely(info != info_expected)) {
		...
		DMUX_LOG_KERR("%s: bam_tx_pool mismatch .next=%p, list_node=%p, ts=%u.%09lu\n",
...
int msm_bam_dmux_write(uint32_t id, struct sk_buff *skb)
{
	 ...
	 */
	if ((skb->len & BAM_DMUX_ALIGN_MASK) &&
	    (skb_tailroom(skb) < (BAM_DMUX_ALIGNMENT - (skb->len & BAM_DMUX_ALIGN_MASK)))) {
		/* revisit, probably dev_alloc_skb and memcpy is effecient */
		new_skb = skb_copy_expand(skb, skb_headroom(skb),
					  BAM_DMUX_ALIGNMENT - (skb->len & BAM_DMUX_ALIGN_MASK),
					  GFP_ATOMIC);
		if (new_skb == NULL) {
		...
		dev_kfree_skb_any(skb);
		skb = new_skb;
		new_skb = NULL;  /* Clear to avoid double-free */
	...
	if (skb->len & BAM_DMUX_ALIGN_MASK)
		skb_put(skb, BAM_DMUX_ALIGNMENT - (skb->len & BAM_DMUX_ALIGN_MASK));
...
static void rx_timer_work_func(struct work_struct *work)
{
	...
	while (bam_connection_is_active) { /* timer loop */
		...
		while (bam_connection_is_active) { /* deplete queue loop */
			if (in_global_reset) {
				BAM_DMUX_LOG("%s: polling exit, global reset detected\n",
						__func__);
				return;
			}
...
static void ul_powerdown_finish(void)
{
...
static void disconnect_to_bam(void)
{
	...
	/*
	 * TODO: Reclaim the CMA block from the modem before the host accesses the
	 * descriptor rings and data buffers during cleanup if required.
	 */
 ...
 */
static inline void set_rx_buffer_ring_pool(int requested_buffs)
{
 ...
 * signal field construction in that function.
 */
static inline void set_dl_mtu(int requested_mtu)
{
