...
struct qcom_mpam_list {
	struct list_head head;
	struct mutex lock;
};

static struct qcom_mpam_list mpam_list;

struct qcom_mpam_msc *qcom_msc_lookup(uint32_t msc_id)
{
	struct qcom_mpam_msc *mpam_msc;
	struct qcom_mpam_msc *found_msc = NULL;

	mutex_lock(&mpam_list.lock);
	list_for_each_entry(mpam_msc, &mpam_list.head, node) {
		if (mpam_msc->msc_id == msc_id) {
			found_msc = mpam_msc;
			break;
		}
	}
	mutex_unlock(&mpam_list.lock);
		...
		uint32_t msc_type)
{
	...
	mutex_lock(&mpam_list.lock);
	list_add_tail(&qcom_msc->node, &mpam_list.head);
	mutex_unlock(&mpam_list.lock);
	return 0;
}
...
static int mpam_msc_probe(struct platform_device *pdev)
{
	...
	mutex_init(&mpam_list.lock);
	INIT_LIST_HEAD(&mpam_list.head);
	return 0;
}

static int mpam_msc_remove(struct platform_device *pdev)
{
	return 0;
}
