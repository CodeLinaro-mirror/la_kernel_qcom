...
struct cpu_mpam_data {
	unsigned long *part_id_free_bitmap;
	unsigned long *monitor_free_bitmap;
...
static struct cpu_mpam_data mpam;
...
static void cpu_mpam_partition_transfer(struct config_item *item, int old, int new)
{
		...
		enum msc_id mscid, char *buf)
{
	...
	if (strnlen(buf, sizeof(temp)) >= sizeof(temp)) {
		pr_err("input parameter too long\n");
		return;
	}
		...
		const char *page, size_t count)
{
	...
	if (!input && monitor_id != INT_MAX) {
		bitmap_clear(mpam.monitor_free_bitmap, monitor_id, 1);
		...
		monitor_id = bitmap_find_next_zero_area(mpam.monitor_free_bitmap,
				MONITOR_MAX, 0, 1, 0);
		...
		bitmap_set(mpam.monitor_free_bitmap, monitor_id, 1);
		...
		char *page)
{
	...
	ssize_t len = 0;
	uint32_t csu_value = 0, mscid;
	uint64_t mbw_value = 0, timestamp = 0, capture_status;
	...
	if (monitor_id != INT_MAX) {
		for (i = 0; i < mpam.mpam_msc_cnt; i++) {
			...
			mpam_mon_data = &mpam.mpam_mon_base[mscid];
			do {
		...
		struct config_item *item)
{
	...
	list_del(&partition->list);

	cpu_mpam_partition_transfer(item, part_id, PARTID_DEFAULT);
	bitmap_clear(mpam.part_id_free_bitmap, part_id, 1);
	if (monitor_id != INT_MAX) {
		bitmap_clear(mpam.monitor_free_bitmap, monitor_id, 1);
		cpu_mpam_disable_monitor(monitor_id, MPAM_TYPE_CSU_MONITOR);
		cpu_mpam_disable_monitor(monitor_id, MPAM_TYPE_MBW_MONITOR);
	...
	/* Release task reference before freeing */
	if (partition->task)
		put_task_struct(partition->task);
		...
		struct config_group *group, const char *name)
{
	...
	part_id = bitmap_find_next_zero_area(mpam.part_id_free_bitmap,
	...
	spin_lock(&mpam.lock);
	list_add_tail(&partition->list, &mpam.partition_head);
...
static DEFINE_PER_CPU(u8, active_partid);

static void cpu_mpam_write_partid(u8 part_id)
{
	u64 reg;
	static u8 old_part_id;

	old_part_id = this_cpu_read(active_partid);

	if (old_part_id == part_id)
		return;

	this_cpu_write(active_partid, part_id);
...
{
	struct cpu_mpam_partition *node, *tmp;
	struct cpu_mpam_partition *default_partition = NULL;
	u8 part_id = 0;

	if (!list_empty(&mpam.partition_head))
		default_partition = list_first_entry(&mpam.partition_head,
				struct cpu_mpam_partition, list);
	if (default_partition)
		part_id = default_partition->part_id;
...
static int cpu_mpam_configfs_init(void)
{
	...
	mpam.part_id_free_bitmap = bitmap_zalloc(PARTID_AVAILABLE, GFP_KERNEL);
	if (!mpam.part_id_free_bitmap) {
		pr_err("Error alloc bitmap\n");
		return -ENOMEM;
	}

	mpam.monitor_free_bitmap = bitmap_zalloc(MONITOR_MAX, GFP_KERNEL);
	if (!mpam.monitor_free_bitmap) {
		pr_err("Error alloc bitmap\n");
		ret = -ENOMEM;
		goto err_free_paritd;
	}
	...
	if (ret) {
		...
		configfs_unregister_group(default_group);
		goto err_mutex;
	}

	cpu_mpam_callback_register(cpu_mpam_switch_task);
...
err_free_monitor:
	bitmap_free(mpam.monitor_free_bitmap);
err_free_paritd:
	bitmap_free(mpam.part_id_free_bitmap);
	return ret;
}

static void cpu_mpam_configfs_remove(void)
{
	cpu_mpam_callback_unregister(cpu_mpam_switch_task);
...
static int cpu_mpam_probe(struct platform_device *pdev)
{
	...
	spin_lock_init(&mpam.lock);
	INIT_LIST_HEAD(&mpam.partition_head);

	mpam.mpam_msc_cnt = of_get_available_child_count(pdev->dev.of_node);
	if (!mpam.mpam_msc_cnt) {
		dev_err(&pdev->dev, "No MSC found\n");
		return -ENODEV;
	}

	mpam_mscs = devm_kcalloc(&pdev->dev, mpam.mpam_msc_cnt,
	...
	for_each_available_child_of_node(pdev->dev.of_node, node) {
		...
		if (ret || mscid >= MSC_MAX)
			continue;
		ret = of_property_read_string(node, "qcom,msc-name", &msc_name_dt);
		if (ret || !msc_name_dt)
			continue;
		...
		if (i >= mpam.mpam_msc_cnt) {
			dev_warn(&pdev->dev, "Too many MSC nodes, ignoring extra entries\n");
			break;
		}
	}
	mpam.mpam_msc_cnt = i;
	...
	if (!res) {
		...
		mpam.can_monitor = false;
	} else {
		mpam.mpam_mon_base = devm_ioremap_resource(&pdev->dev, res);
		if (IS_ERR_OR_NULL(mpam.mpam_mon_base)) {
			dev_err(&pdev->dev, "Error ioremap mpam_mon_base\n");
			return -ENODEV;
		}
		mpam.can_monitor = true;
	}
	if (mpam.can_monitor) {
		for (i = 0; i < mpam.mpam_msc_cnt; i++) {
