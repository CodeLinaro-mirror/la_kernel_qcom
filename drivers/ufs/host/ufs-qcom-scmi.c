...
static int ufs_qcom_resume(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	if (err < 0)
		dev_err(hba->dev, "Failed to resume, ret:%d\n", err);
		return err;
	}
...
static void ufs_qcom_qos_init(struct ufs_hba *hba)
{
	...
	qcg = kcalloc(qr->num_groups, sizeof(*qcg), GFP_KERNEL);
	if (!qcg) {
		kfree(qr);
		host->ufs_qos = NULL;
		return;
...
static int ufs_qcom_init(struct ufs_hba *hba)
{
	...
	if (err) {
		dev_err(hba->dev, "Failed to initialize ICE, ret=%d\n", err);
	...
	/* Balance PM usage count for UFS device */
