...
static int inlinecrypt_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	u8 raw_key[BLK_CRYPTO_MAX_HW_WRAPPED_KEY_SIZE];
	...
	if (key_size > 2 * BLK_CRYPTO_MAX_HW_WRAPPED_KEY_SIZE || key_size % 2) {
		ti->error = "Invalid keysize";
		err = -EINVAL;
		goto bad;
	}
	key_size /= 2;
	if (hex2bin(raw_key, argv[1], key_size) != 0) {
	...
	err = blk_crypto_init_key(&ctx->key, raw_key, key_size,
