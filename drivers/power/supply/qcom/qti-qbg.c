...
static int qbg_determine_pon_soc(struct qti_qbg *chip)
{
	...
	qbg_dbg(chip, QBG_DEBUG_PON, "Shutdown: SOC=%d OCV=%duV Ibat:%duA timediff=%lusecs, time_now=%lldsecs\n",
...
static int qbg_init_psy(struct qti_qbg *chip)
{
	...
	if (IS_ERR_OR_NULL(chip->qbg_psy)) {
		pr_err("Failed to register qbg_psy, rc = %ld\n",
...
static int qbg_iio_fwnode_xlate(struct iio_dev *indio_dev,
...
static const struct iio_info qbg_iio_info = {
	...
	.fwnode_xlate	= qbg_iio_fwnode_xlate,
};
...
static ssize_t qbg_context_show(const struct class *c,
...
static ssize_t qbg_context_store(const struct class *c,
						...
						unsigned long arg)
{
	...
	case QBG_QBGIOCXCFG:
		...
		for (i = 0; i < QBG_STATE_MAX; i++) {
			...
			qbg_dbg(chip, QBG_DEBUG_DEVICE, "QBGIOCXCFG: sample_time_us[%d]:%llu\n",
	...
	case QBG_QBGIOCXEPW:
		...
		qbg_dbg(chip, QBG_DEBUG_SDAM, "Essential params written, time:%lld secs\n",
					rtc_sec);
...
static void qti_qbg_remove(struct platform_device *pdev)
{
