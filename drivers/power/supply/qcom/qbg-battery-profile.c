	...
	struct qbg_battery_data *battery)
{
	...
	battery->battery_class = class_create("qbg_battery");
	if (IS_ERR_OR_NULL(battery->battery_class)) {
		pr_err("Failed to create qbg-battery class (%ld)\n",
	...
	if (IS_ERR_OR_NULL(battery->battery_device)) {
		pr_err("Failed to create battery_device device (%ld)\n",
...
int qbg_lookup_soc_ocv(struct qbg_battery_data *battery, int *pon_soc, int ocv, bool charging)
{
	...
	*pon_soc = -EINVAL;
	...
	if (*pon_soc == -EINVAL) {
		pr_debug("%d ocv wasn't found in the LUT returning 100 percent\n", ocv);
		*pon_soc = 10000;
	}
