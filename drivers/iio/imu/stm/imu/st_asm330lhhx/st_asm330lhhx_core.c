...
#ifdef CONFIG_ENABLE_ASM_ACC_GYRO_BUFFERING
static void st_asm330lhh_enable_acc_gyro(struct st_asm330lhhx_hw *hw)
{
	int i = 0;
	struct st_asm330lhhx_sensor *sensor;
	int  acc_gain = ST_ASM330LHHX_ACC_FS_4G_GAIN;
	int  gyro_gain = ST_ASM330LHHX_GYRO_FS_125_GAIN;
	int  delay;
	...
	for (i = 0; i < ST_ASM330LHHX_ID_MAX; i++) {
		...
		if (sensor->id == ST_ASM330LHHX_ID_ACC) {
			...
			usleep_range(delay, 2 * delay);
			st_asm330lhhx_set_odr(sensor, false, sensor->odr, sensor->uodr);
			usleep_range(delay, 2 * delay);
			st_asm330lhhx_update_watermark(sensor,
				sensor->watermark);
			usleep_range(delay, 2 * delay);
			st_asm330lhhx_update_fifo(sensor, true);
			usleep_range(delay, 2 * delay);
