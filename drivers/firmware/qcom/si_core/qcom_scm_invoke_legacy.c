	...
	int *result, u64 *response_type, unsigned int *data)
{
	...
	do {
		...
		if (!(oic->flags & OIC_FLAG_BUSY)) {
			qtee_shmbridge_flush_shm_buf(&oic->in.shm);
			qtee_shmbridge_flush_shm_buf(&oic->out.shm);
			ret = qcom_scm_invoke_smc_legacy(oic->in.paddr,
				...
				response_type,
				data);
			qtee_shmbridge_inv_shm_buf(&oic->in.shm);
			qtee_shmbridge_inv_shm_buf(&oic->out.shm);
