...
EXPORT_SYMBOL_GPL(qcom_scm_call_ethqos_configure);
	...
	u32 loopback_mode, u32 if_mode)
{
	...
	ret = qcom_scm_call(__scm->dev, &desc, &res);
	pr_info("scm loopback configure 0x%x done = 0x%x\n", emac_base_addr, ret);
...
EXPORT_SYMBOL_GPL(qcom_scm_call_loopback_configure);
...
EXPORT_SYMBOL_GPL(qcom_scm_call_iomacro_dump);
	...
	u32 *maxspeed)
{
	...
	pr_info("scm get max_spd ret=0x%x res0 = %llu res1 =%llu res2 = %llu\n",
...
EXPORT_SYMBOL_GPL(qcom_scm_call_get_emac_maxspeed);
...
int __qcom_scm_ipa_intr_config(struct device *dev, u32 emac_base_addr, u32 value)
{
	...
	ret = qcom_scm_call(__scm->dev, &desc, &res);
	pr_info("scm ipa intr 0x%x config = 0x%x done = 0x%x\n", emac_base_addr, value, ret);
...
EXPORT_SYMBOL_GPL(qcom_scm_call_ipa_intr_config);
