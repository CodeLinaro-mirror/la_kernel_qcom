					 ...
					 const struct ktermios *old)
{
	...
	ret = geni_se_clk_freq_match(&port->se, baud * sampling_rate, &clk_idx,
				     &clk_rate, false);
	if (ret) {
	...
	serial_trace_log(port->se.dev,
			 "baud: %u, desired_rate: %u, clk_rate: %lu clk_div: %u clk_idx: %u\n",
			 baud, baud * sampling_rate, clk_rate, clk_div, clk_idx);
