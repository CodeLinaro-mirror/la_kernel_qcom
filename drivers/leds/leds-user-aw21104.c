...
struct aw21104_user_led {
	struct aw21104_user *chip;
...
struct aw21104_user {
	...
	struct aw21104_user_led leds[AW2013_MAX_LEDS];
...
static const struct regmap_config aw21104_user_led_regmap_config = {
	.reg_bits = 8,
	.val_bits = 8,
					...
					struct device_attribute *attr, char *buf)
{
	...
	chip = (struct aw21104_user *)i2c_get_clientdata(client);
...
static struct attribute *aw21104_user_led_attrs[] = {
	&dev_attr_user_led_reg.attr,
	&dev_attr_user_led_val.attr,
	NULL
};

static const struct attribute_group aw21104_user_led_attr_group = {
	.attrs = aw21104_user_led_attrs,
};

int aw21104_user_led_probe(struct i2c_client *client)
{
	struct aw21104_user *chip;
	int ret;
	unsigned int chipid;
	...
	chip->regmap = devm_regmap_init_i2c(client, &aw21104_user_led_regmap_config);
	...
	ret = sysfs_create_group(&client->dev.kobj, &aw21104_user_led_attr_group);
	if (ret) {
	...
	return 0;
error2:
	sysfs_remove_group(&client->dev.kobj, &aw21104_user_led_attr_group);
...
static void aw21104_user_led_remove(struct i2c_client *client)
{
	if (client)
		sysfs_remove_group(&client->dev.kobj, &aw21104_user_led_attr_group);
}

static const struct of_device_id aw21104_user_led_match_table[] = {
	{ .compatible = "awinic,rgbw-user-led", },
	{ /* sentinel */ }
};

MODULE_DEVICE_TABLE(of, aw21104_user_led_match_table);

static struct i2c_driver aw21104_user_led_driver = {
	.driver = {
		.name = "rgbw-user-led",
		.of_match_table = aw21104_user_led_match_table,
	},
	.probe = aw21104_user_led_probe,
	.remove = aw21104_user_led_remove,
};

module_i2c_driver(aw21104_user_led_driver);

MODULE_DESCRIPTION("AWINIC AW21104 RGBW USER LED driver");
MODULE_LICENSE("GPL");
