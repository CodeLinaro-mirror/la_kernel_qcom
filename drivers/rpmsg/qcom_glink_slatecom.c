 ...
 * @rx_defer_work:	api to defer and handle rx data
 * @tx_lock:	synchronizes operations on the tx fifo
 * @idr_lock:	synchronizes @lcids and @rcids modifications
 * @lcids:	idr of all channels with a known local channel id
 * @rcids:	idr of all channels with a known remote channel id
 * @features:	local glink features
 * @activity_cnt:	activity counter
 * @in_reset:	indicates that remote processor is in reset
 * @ws:		wake up source
 * @ilc:	ipc logging context reference
 * @sent_read_notify:	flag to check cmd sent or not
 * @fifo_fill:	tracks available size of rx and tx fifo
 * @fifo_size:	holds master and slave fifo sizes
 * @tx_avail_lock:	lock acquired while operating on tx fifo
 * @kworker:	worker thread to handles rx tasks
 * @slatecom_status:	local slatecom event status
 * @slatecom_config:	local slatecom channel configuration
 * @slatecom_handle:	pointer to slatecom handle
 * @water_mark_reached: indicates when tx fifo is full
 ...
 * @rx_close_ack:	structure to receive rx close acknowledgment
 * @send_close_ack:	structure to send channel close acknowledgment
 * @intent_req_lock:	Synchronises multiple intent requests
 * @intent_req_result:	Result of intent request
 * @intent_req_acked:	flag indicating intent request has been acknowledged
 * @intent_req_completed:	flag indicating intent req has been completed
 * @intent_req_ack:	Wait queue head for pending acks of intent requests
					...
					void *data, size_t dlen)
{
	...
	if (*(uint8_t *)(data) != SLATECOM_CMD_TX_SHORT_DATA)
		*(uint32_t *)(data + TX_COUNTER_OFFSET) = ++(glink->tx_counter);
	else
 ...
 * @rx_pkt_info:	Edge to process commands on.
 *
...
static void glink_slatecom_remove(struct platform_device *pdev)
{
	struct glink_slatecom *glink = platform_get_drvdata(pdev);
	int ret;
...
static struct platform_driver glink_slatecom_driver = {
	...
	.remove_new = glink_slatecom_remove,
	.driver = {
...
module_platform_driver(glink_slatecom_driver);
