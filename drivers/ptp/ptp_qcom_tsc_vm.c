...
{
	unsigned long flags;
	struct qcom_ptp_tsc_vm *timer = container_of(ptp, struct qcom_ptp_tsc_vm, ptp_clock_info);
...
static void qcom_ptp_tsc_vm_remove(struct platform_device *pdev)
{
...
static int qcom_ptp_tsc_vm_probe(struct platform_device *pdev)
{
	...
	timer->tsc_nsec_update = of_property_read_bool(pdev->dev.of_node, "qcom,tsc-nsec-update");
	...
	pr_debug("TSC PTP clock registered in VM\n");
