...
void qpic_lcdc_reset(struct qpic_display_data *qpic_display)
{
	...
	while (((QPIC_INP(qpic_display, QPIC_REG_QPIC_LCDC_STTS) & (1 << 8)) == 0)) {
		if ((u32)ktime_to_ms(ktime_get()) > time_end) {
			DRM_ERROR("%s reset not finished\n", __func__);
			break;
		}
		...
		bool active)
{
	...
	if (!IS_ERR_OR_NULL(pin_state)) {
		rc = pinctrl_select_state(qpic_panel_io->pin_res.pinctrl,
			pin_state);
		if (rc)
			DRM_ERROR("%s: can not set %s pins\n", __func__,
				active ? QPIC_PINCTRL_STATE_DEFAULT
				: QPIC_PINCTRL_STATE_SLEEP);
	} else {
		DRM_ERROR("%s: invalid '%s' pinstate\n", __func__,
				active ? QPIC_PINCTRL_STATE_DEFAULT
				: QPIC_PINCTRL_STATE_SLEEP);
	}
...
static int msm_qpic_bus_set_vote(struct qpic_display_data *qpic_display, bool vote)
{
	...
	DRM_DEBUG_DRIVER("vote:%d qpic-display-data-bus ab:%llu ib:%llu\n",
			vote, vec->ab, vec->ib);
	ddr_rc = icc_set_bw(bvd->data_bus_hdl, vec->ab, vec->ib);
	if (ddr_rc)
		DRM_ERROR("icc_set() failed\n");
	else
		bvd->curr_vote = vote;
			...
			u32 cmd, u32 len, u8 *param)
{
	...
	while (len > 0)  {
		...
		if (ret)
			DRM_ERROR("failed to submit command %x ret %d\n",
				cmd, ret);
		phys_addr += block_len;
		len -= block_len;
	}
	ret = wait_for_completion_timeout(&qpic_display->qpic_endpt.completion,
...
static int qpic_lcdc_wait_for_eof(struct qpic_display_data *qpic_display)
{
	...
	if (use_irq) {
		...
		while (1) {
			...
			usleep_range(10, 11);
			if (ktime_to_ms(ktime_get()) > time_end) {
				DRM_ERROR("%s wait for eof time out\n", __func__);
				ret = -EBUSY;
				break;
			}
		}
...
int qpic_init_sps(struct qpic_display_data *qpic_display)
{
	...
	if (sps_phy2h(bam.phys_addr, &bam_handle)) {
		if (sps_register_bam_device(&bam, &bam_handle)) {
			DRM_ERROR("%s bam_handle is NULL\n", __func__);
			rc = -ENOMEM;
			goto out;
		}
	}
	...
	if (!pipe_handle) {
		DRM_ERROR("sps_alloc_endpoint() failed\n");
		rc = -ENOMEM;
		goto out;
	}
	...
	if (rc) {
		DRM_ERROR("sps_get_config() failed %d\n", rc);
		goto free_endpoint;
	}
	...
	if (!sps_config->desc.base) {
		DRM_ERROR("dmam_alloc_coherent() failed for size %x\n",
				sps_config->desc.size);
		rc = -ENOMEM;
		goto free_endpoint;
	}
	...
	if (rc) {
		DRM_ERROR("sps_connect() failed %d\n", rc);
		goto free_endpoint;
	}
	...
	if (rc) {
		DRM_ERROR("sps_register_event() failed %d\n", rc);
		goto sps_disconnect;
	}
		...
		u32 *data, u32 total_bytes)
{
	...
	if (status) {
		DRM_ERROR("Failed to start memory write\n");
		return status;
	}
...
static int qpic_panel_regulator_init(struct qpic_panel_io_desc *panel_io)
{
	...
	if (panel_io->vdd_vreg) {
		rc = regulator_set_voltage(panel_io->vdd_vreg,
			1800000, 1800000);
		if (rc) {
			DRM_ERROR("iovdd_vreg->set_voltage failed, rc=%d\n", rc);
			return -EINVAL;
		}
	}
	if (panel_io->avdd_vreg) {
		rc = regulator_set_voltage(panel_io->avdd_vreg,
			2704000, 2704000);
		if (rc) {
			DRM_ERROR("avdd_vreg->set_voltage failed, rc=%d\n", rc);
			return -EINVAL;
		}
	}
...
static void panel_io_disable(struct qpic_panel_io_desc *qpic_panel_io)
{
	if (qpic_display_pinctrl_set_state(qpic_panel_io, false))
		DRM_WARN("%s panel on: pinctrl not enabled\n", __func__);
...
static int panel_io_enable(struct qpic_panel_io_desc *qpic_panel_io)
{
	...
	if (qpic_panel_io->vdd_vreg) {
		...
		if (rc) {
			DRM_ERROR("enable vdd failed, rc=%d\n", rc);
			return -ENODEV;
		}
	}
	...
	if (qpic_panel_io->avdd_vreg) {
		...
		if (rc) {
			DRM_ERROR("enable avdd failed, rc=%d\n", rc);
			goto power_io_error;
		}
	}
...
int qpic_display_on(struct qpic_display_data *qpic_display)
{
	...
	if (qpic_display->panel_on && !qpic_display->is_panel_on) {
		...
		DRM_INFO("panel on function is not specified or panel already on\n");
		return 0;
	}
...
DEFINE_DRM_GEM_DMA_FOPS(qpic_display_fops);
...
static void qpic_display_fb_mark_dirty(struct drm_framebuffer *fb, struct drm_rect *rect)
{
	...
	if (!qpic_display->is_qpic_on || !qpic_display->is_panel_on) {
		DRM_INFO("%s: qpic or panel is not enabled\n", __func__);
		return;
	}

	dma_obj = drm_fb_dma_get_gem_obj(fb, 0);
	if (!dma_obj) {
		DRM_ERROR("failed to get gem obj\n");
		return;
	}
	...
	qpic_send_frame(qpic_display, rect->x1, rect->y1,
		rect->x2 - 1, rect->y2 - 1,
		use_bam ? (u32 *)dma_obj->dma_addr:(u32 *)dma_obj->vaddr, size);
		...
		struct qpic_panel_io_desc *qpic_panel_io)
{
	...
	if (IS_ERR_OR_NULL(qpic_panel_io->pin_res.pinctrl)) {
		DRM_ERROR("%s: failed to get pinctrl\n", __func__);
		return PTR_ERR(qpic_panel_io->pin_res.pinctrl);
	}
	...
	if (IS_ERR_OR_NULL(qpic_panel_io->pin_res.gpio_state_active))
		DRM_WARN("%s: cannot get default pinstate\n", __func__);

	qpic_panel_io->pin_res.gpio_state_suspend
		= pinctrl_lookup_state(qpic_panel_io->pin_res.pinctrl,
				QPIC_PINCTRL_STATE_SLEEP);
	if (IS_ERR_OR_NULL(qpic_panel_io->pin_res.gpio_state_suspend))
		DRM_WARN("%s: cannot get sleep pinstate\n", __func__);
	...
	struct qpic_panel_io_desc *qpic_panel_io)
{
	...
	if (rc) {
		DRM_ERROR("%s: failed to get pin resources\n", __func__);
		return rc;
	}
	...
	if (!gpio_is_valid(rst_gpio))
		DRM_WARN("%s: reset gpio not specified\n", __func__);
	else
		qpic_panel_io->rst_gpio = rst_gpio;

	if (!gpio_is_valid(cs_gpio))
		DRM_WARN("%s: cs gpio not specified\n", __func__);
	else
		qpic_panel_io->cs_gpio = cs_gpio;

	if (!gpio_is_valid(ad8_gpio))
		DRM_WARN("%s: ad8 gpio not specified\n", __func__);
	else
...
int qpic_display_alloc_cmd_buf(struct qpic_display_data *qpic_display)
{
	...
	if (!qpic_display->cmd_buf_virt) {
		DRM_ERROR("%s cmd buf allocation failed\n", __func__);
		return -ENOMEM;
	}
...
int qpic_display_get_resource(struct qpic_display_data *qpic_display)
{
	...
	if (!res) {
		DRM_ERROR("unable to get QPIC reg base address\n");
		return -ENOMEM;
	}
	...
	if (unlikely(!qpic_display->qpic_base)) {
		DRM_ERROR("unable to map MDSS QPIC base\n");
		return -ENOMEM;
	}
	...
	if (IS_ERR(qpic_display->qpic_clk)) {
		...
		DRM_ERROR("%s: Can't find core_clk\n", __func__);
		return -ENODEV;
	}
...
static int qpic_display_probe(struct platform_device *pdev)
{
	...
	if (!pdev->dev.of_node) {
		DRM_ERROR("qpic driver only supports device tree probe\n");
		return -EOPNOTSUPP;
	}

	qpic_display = devm_drm_dev_alloc(&pdev->dev, &qpic_drm_driver,
				 struct qpic_display_data, drm_dev);
	if (IS_ERR(qpic_display))
		return PTR_ERR(qpic_display);
	...
	if (rc) {
		DRM_ERROR("failed to get panel config\n");
		goto out;
	}
	...
	if (rc) {
		DRM_ERROR("qpic display get resource failed, rc = %d\n", rc);
		goto out;
	}

	rc = qpic_display_io_init(pdev, &qpic_display->panel_io);
	if (rc) {
		DRM_ERROR("qpic display IO init failed, rc = %d\n", rc);
		goto out;
	}

	rc = qpic_panel_regulator_init(&qpic_display->panel_io);
	if (rc) {
		DRM_ERROR("qpic panel regulator init failed, rc = %d\n", rc);
		goto out;
	}

	rc = qpic_display_io_request(&qpic_display->panel_io);
	if (rc) {
		DRM_ERROR("qpic display io request failed, rc = %d\n", rc);
		goto out;
	}

	rc = qpic_display_bus_register(pdev, qpic_display);
	if (rc) {
		DRM_ERROR("qpic display bus register failed, rc = %d\n", rc);
		goto out;
	}

	rc = qpic_display_alloc_cmd_buf(qpic_display);
	if (rc) {
		DRM_ERROR("qpic display allocate cmd buffer failed, rc = %d\n", rc);
		goto bus_unregister;
	}
	...
	rc = drmm_mode_config_init(drm_dev);
	if (rc) {
		DRM_ERROR("drm mode config init failed, rc = %d\n", rc);
		goto err_put;
	}
	...
	if (rc) {
		DRM_ERROR("qpic display connector init failed, rc = %d\n", rc);
		goto err_put;
	}
...
static int __init qpic_display_driver_init(void)
{
	...
	if (ret)
		DRM_ERROR("qpic_display_register_driver() failed, ret=%d\n", ret);
	return ret;
}
