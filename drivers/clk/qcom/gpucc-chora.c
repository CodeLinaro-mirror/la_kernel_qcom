...
#include <dt-bindings/clock/qcom,chora-gpucc.h>
...
static struct clk_vdd_class *gpu_cc_chora_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_rcg2 gpu_cc_gmu_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = gpu_cc_chora_regulators,
		.num_vdd_classes = ARRAY_SIZE(gpu_cc_chora_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 350000000,
			[VDD_LOW] = 650000000,
...
static struct clk_regmap *gpu_cc_chora_clocks[] = {
...
static struct gdsc *gpu_cc_chora_gdscs[] = {
...
static const struct qcom_reset_map gpu_cc_chora_resets[] = {
...
static const struct regmap_config gpu_cc_chora_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
	.max_register = 0x95e8,
...
static const struct qcom_cc_desc gpu_cc_chora_desc = {
	.config = &gpu_cc_chora_regmap_config,
	.clks = gpu_cc_chora_clocks,
	.num_clks = ARRAY_SIZE(gpu_cc_chora_clocks),
	.resets = gpu_cc_chora_resets,
	.num_resets = ARRAY_SIZE(gpu_cc_chora_resets),
	.clk_regulators = gpu_cc_chora_regulators,
	.num_clk_regulators = ARRAY_SIZE(gpu_cc_chora_regulators),
	.gdscs = gpu_cc_chora_gdscs,
	.num_gdscs = ARRAY_SIZE(gpu_cc_chora_gdscs),
};

static const struct qcom_cc_desc gx_clkctl_chora_desc = {
	.config = &gx_clkctl_regmap_config,
	.gdscs = gx_clkctl_gdscs,
	.num_gdscs = ARRAY_SIZE(gx_clkctl_gdscs),
};

static const struct of_device_id gpu_cc_chora_match_table[] = {
	{ .compatible = "qcom,chora-gpucc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gpu_cc_chora_match_table);

static int gpu_cc_chora_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gpu_cc_chora_desc);
	if (IS_ERR(regmap))
	...
	regmap_update_bits(regmap, 0x9010, BIT(0), BIT(0));
	...
	ret = qcom_cc_really_probe(&pdev->dev, &gpu_cc_chora_desc, regmap);
	if (ret) {
...
static void gpu_cc_chora_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gpu_cc_chora_desc);
}

static struct platform_driver gpu_cc_chora_driver = {
	.probe = gpu_cc_chora_probe,
	.driver = {
		.name = "gpucc-chora",
		.of_match_table = gpu_cc_chora_match_table,
		.sync_state = gpu_cc_chora_sync_state,
	},
};

static const struct of_device_id gx_clkctl_chora_match_table[] = {
	{ .compatible = "qcom,chora-gx_clkctl" },
	{ }
};
MODULE_DEVICE_TABLE(of, gx_clkctl_chora_match_table);

static int gx_clkctl_chora_probe(struct platform_device *pdev)
{
	...
	regmap = qcom_cc_map(pdev, &gx_clkctl_chora_desc);
	if (IS_ERR(regmap)) {
		ret = PTR_ERR(regmap);
		goto err;
	}

	ret = qcom_cc_really_probe(&pdev->dev, &gx_clkctl_chora_desc, regmap);
	if (ret) {
...
static struct platform_driver gx_clkctl_chora_driver = {
	.probe = gx_clkctl_chora_probe,
	.driver = {
		.name = "gx_clkctl-chora",
		.of_match_table = gx_clkctl_chora_match_table,
	},
};

static int __init gpu_cc_chora_init(void)
{
	int ret;

	ret = platform_driver_register(&gpu_cc_chora_driver);
	if (ret)
		return ret;

	return platform_driver_register(&gx_clkctl_chora_driver);
}
subsys_initcall(gpu_cc_chora_init);

static void __exit gpu_cc_chora_exit(void)
{
	platform_driver_unregister(&gx_clkctl_chora_driver);
	platform_driver_unregister(&gpu_cc_chora_driver);
}
module_exit(gpu_cc_chora_exit);

MODULE_DESCRIPTION("QTI GPUCC CHORA Driver");
MODULE_LICENSE("GPL");
