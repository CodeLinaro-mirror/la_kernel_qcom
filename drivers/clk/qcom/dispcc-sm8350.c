...
static struct clk_vdd_class *disp_cc_sm8350_regulators[] = {
	&vdd_mm,
};
...
static struct qcom_cc_desc disp_cc_sm8350_desc = {
	...
	.clk_regulators = disp_cc_sm8350_regulators,
	.num_clk_regulators = ARRAY_SIZE(disp_cc_sm8350_regulators),
};
...
static int disp_cc_sm8350_probe(struct platform_device *pdev)
{
	...
	ret = qcom_cc_really_probe(&pdev->dev, &disp_cc_sm8350_desc, regmap);
	if (ret) {
		if (ret != -EPROBE_DEFER)
...
static void disp_cc_sm8350_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &disp_cc_sm8350_desc);
}
...
static struct platform_driver disp_cc_sm8350_driver = {
	...
	.driver = {
		...
		.sync_state = disp_cc_sm8350_sync_state,
