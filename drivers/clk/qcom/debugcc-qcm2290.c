...
static struct clk_hw *debugcc_qcm2290_hws[] = {
...
static const struct of_device_id clk_debug_match_table[] = {
	{ .compatible = "qcom,qcm2290-debugcc" },
	{ }
};

static int clk_debug_qcm2290_probe(struct platform_device *pdev)
{
	struct clk *clk;
	int ret = 0, i;
	...
	for (i = 0; i < ARRAY_SIZE(debugcc_qcm2290_hws); i++) {
		clk = devm_clk_register(&pdev->dev, debugcc_qcm2290_hws[i]);
		if (IS_ERR(clk)) {
			dev_err(&pdev->dev, "Unable to register %s, err:(%ld)\n",
				clk_hw_get_name(debugcc_qcm2290_hws[i]),
				PTR_ERR(clk));
			return PTR_ERR(clk);
		}
	}
...
static struct platform_driver clk_debug_driver = {
	.probe = clk_debug_qcm2290_probe,
	.driver = {
		.name = "qcm2290-debugcc",
		.of_match_table = clk_debug_match_table,
	},
};

static int __init clk_debug_qcm2290_init(void)
{
	return platform_driver_register(&clk_debug_driver);
}
fs_initcall(clk_debug_qcm2290_init);

MODULE_DESCRIPTION("QTI DEBUG CC QCM2290 Driver");
MODULE_LICENSE("GPL");
