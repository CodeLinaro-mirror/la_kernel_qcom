...
/* Always ON clocks GPU_CC_AHB_CLK */
static struct critical_clk_offset critical_clk_list[] = {
	{ .offset = 0x4078, .mask = BIT(0) },
};
...
static struct qcom_cc_desc gpu_cc_monaco_desc = {
...
static int gpu_cc_monaco_probe(struct platform_device *pdev)
{
	...
	ret = regmap_update_bits(regmap, gpu_cc_cx_gmu_clk.clkr.enable_reg,
				       mask, value);
	if (ret) {
		dev_err(&pdev->dev, "Failed to configure gpu_cc_cx_gmu_clk clocks: %d\n", ret);
		return ret;
	}

	ret = register_qcom_clks_pm(pdev, false, &gpu_cc_monaco_desc);
	if (ret) {
		dev_err(&pdev->dev, "Failed to register gpucc_pm_rt_ops clocks: %d\n", ret);
		return ret;
	}

	/* Enabling always ON clocks */
	clk_restore_critical_clocks(&pdev->dev);
	ret = qcom_cc_really_probe(&pdev->dev, &gpu_cc_monaco_desc, regmap);
	if (ret) {
