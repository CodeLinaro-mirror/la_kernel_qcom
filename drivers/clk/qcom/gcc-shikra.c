...
static struct clk_vdd_class *gcc_shikra_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static const struct qcom_cc_desc gcc_shikra_desc = {
	...
	.clk_regulators = gcc_shikra_regulators,
	.num_clk_regulators = ARRAY_SIZE(gcc_shikra_regulators),
...
static int gcc_shikra_probe(struct platform_device *pdev)
{
	...
	regmap = qcom_cc_map(pdev, &gcc_shikra_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_register_rcg_dfs(regmap, gcc_shikra_dfs_clocks,
				       ARRAY_SIZE(gcc_shikra_dfs_clocks));
	if (ret)
		return ret;
	...
	regmap_update_bits(regmap, 0x7900c, BIT(13), BIT(13));
	...
	if (ret) {
		dev_err_probe(&pdev->dev, ret, "Failed to register GCC clocks\n");
		return ret;
	}
...
static void gcc_shikra_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_shikra_desc);
}
...
static struct platform_driver gcc_shikra_driver = {
	...
	.driver = {
		...
		.sync_state = gcc_shikra_sync_state,
	},
};
