...
static struct clk_hw *debugcc_sm8450_hws[] = {
...
static const struct of_device_id clk_debug_match_table[] = {
	{ .compatible = "qcom,sm8450-debugcc" },
	{ }
};

static int clk_debug_sm8450_probe(struct platform_device *pdev)
{
	struct clk *clk;
	int ret = 0, i;
	...
	for (i = 0; i < ARRAY_SIZE(debugcc_sm8450_hws); i++) {
		clk = devm_clk_register(&pdev->dev, debugcc_sm8450_hws[i]);
		if (IS_ERR(clk)) {
			dev_err(&pdev->dev, "Unable to register %s, err:(%ld)\n",
				qcom_clk_hw_get_name(debugcc_sm8450_hws[i]),
				PTR_ERR(clk));
			return PTR_ERR(clk);
		}
	}
...
static int __init clk_debug_sm8450_init(void)
{
	return platform_driver_register(&clk_debug_driver);
}
fs_initcall(clk_debug_sm8450_init);

MODULE_DESCRIPTION("QTI DEBUG CC SM8450 Driver");
MODULE_LICENSE("GPL");
