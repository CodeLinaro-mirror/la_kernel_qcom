...
static struct clk_vdd_class *disp_cc_shikra_regulators[] = {
	&vdd_cx,
};
...
static const struct qcom_cc_desc disp_cc_shikra_desc = {
	...
	.clk_regulators = disp_cc_shikra_regulators,
	.num_clk_regulators = ARRAY_SIZE(disp_cc_shikra_regulators),
...
static int disp_cc_shikra_probe(struct platform_device *pdev)
{
	...
	regmap = qcom_cc_map(pdev, &disp_cc_shikra_desc);
	if (IS_ERR(regmap))
	 ...
	 *	disp_cc_xo_clk
	 */
	regmap_update_bits(regmap, 0x6068, BIT(0), BIT(0));
	regmap_update_bits(regmap, 0x604c, BIT(0), BIT(0));

	ret = qcom_cc_really_probe(&pdev->dev, &disp_cc_shikra_desc, regmap);
	if (ret)
...
static void disp_cc_shikra_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &disp_cc_shikra_desc);
}
...
static struct platform_driver disp_cc_shikra_driver = {
	...
	.driver = {
		...
		.sync_state = disp_cc_shikra_sync_state,
	},
};
