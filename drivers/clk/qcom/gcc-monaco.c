...
static int gcc_monaco_probe(struct platform_device *pdev)
{
	...
	ret = regmap_update_bits(regmap, 0x3600c, 0xF00, 0xF00);
	if (ret) {
		dev_err(&pdev->dev, "Failed to configure GPU MEMNOC GFX clock WAKEUP cycle: %d\n",
			ret);
		return ret;
	}
	...
	if (ret) {
		dev_err(&pdev->dev, "Failed to register gcc_pm_rt_ops clocks: %d\n", ret);
		return ret;
	}

	/* Enabling always ON clocks */
	clk_restore_critical_clocks(&pdev->dev);
	ret = qcom_cc_really_probe(&pdev->dev, &gcc_monaco_desc, regmap);
	if (ret) {
