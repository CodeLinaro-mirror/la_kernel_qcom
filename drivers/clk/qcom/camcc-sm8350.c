...
#include <dt-bindings/clock/qcom,camcc-sm8350.h>
...
static struct clk_vdd_class *cam_cc_sm8350_regulators[] = {
	&vdd_mm,
	&vdd_mx,
};
...
static struct clk_regmap *cam_cc_sm8350_clocks[] = {
...
static const struct qcom_reset_map cam_cc_sm8350_resets[] = {
...
static const struct regmap_config cam_cc_sm8350_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
	.max_register = 0xf008,
...
static struct qcom_cc_desc cam_cc_sm8350_desc = {
	.config = &cam_cc_sm8350_regmap_config,
	.clks = cam_cc_sm8350_clocks,
	.num_clks = ARRAY_SIZE(cam_cc_sm8350_clocks),
	.resets = cam_cc_sm8350_resets,
	.num_resets = ARRAY_SIZE(cam_cc_sm8350_resets),
	.clk_regulators = cam_cc_sm8350_regulators,
	.num_clk_regulators = ARRAY_SIZE(cam_cc_sm8350_regulators),
};

static const struct of_device_id cam_cc_sm8350_match_table[] = {
	{ .compatible = "qcom,sm8350-camcc" },
	{ .compatible = "qcom,sm8350-camcc-v2" },
	{ }
};
MODULE_DEVICE_TABLE(of, cam_cc_sm8350_match_table);

static void cam_cc_sm8350_fixup_sm8350v2(struct regmap *regmap)
{
	clk_zonda_5lpe_pll_configure(&cam_cc_pll2, regmap,
		&cam_cc_pll2_config_sm8350_v2);
}

static int cam_cc_sm8350_fixup(struct platform_device *pdev,
	struct regmap *regmap)
{
	...
	if (!strcmp(compat, "qcom,sm8350-camcc-v2"))
		cam_cc_sm8350_fixup_sm8350v2(regmap);

	return 0;
}

static int cam_cc_sm8350_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &cam_cc_sm8350_desc);
	if (IS_ERR(regmap)) {
		dev_err(&pdev->dev, "Failed to map cam CC registers\n");
		return PTR_ERR(regmap);
	}

	ret = qcom_cc_runtime_init(pdev, &cam_cc_sm8350_desc);
	if (ret)
		return ret;
	...
	ret = cam_cc_sm8350_fixup(pdev, regmap);
	if (ret)
		return ret;
	...
	ret = qcom_cc_really_probe(&pdev->dev, &cam_cc_sm8350_desc, regmap);
	if (ret) {
	...
	dev_err(&pdev->dev, "Registered CAM CC clocks\n");

	return ret;
}

static void cam_cc_sm8350_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &cam_cc_sm8350_desc);
}

static const struct dev_pm_ops cam_cc_sm8350_pm_ops = {
	SET_RUNTIME_PM_OPS(qcom_cc_runtime_suspend, qcom_cc_runtime_resume, NULL)
	SET_LATE_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
				     pm_runtime_force_resume)
};

static struct platform_driver cam_cc_sm8350_driver = {
	.probe = cam_cc_sm8350_probe,
	.driver = {
		.name = "sm8350-cam_cc",
		.of_match_table = cam_cc_sm8350_match_table,
		.sync_state = cam_cc_sm8350_sync_state,
		.pm = &cam_cc_sm8350_pm_ops,
	},
};

static int __init cam_cc_sm8350_init(void)
{
	return platform_driver_register(&cam_cc_sm8350_driver);
}
subsys_initcall(cam_cc_sm8350_init);

static void __exit cam_cc_sm8350_exit(void)
{
	platform_driver_unregister(&cam_cc_sm8350_driver);
}
module_exit(cam_cc_sm8350_exit);

MODULE_DESCRIPTION("QTI CAM_CC SM8350 Driver");
MODULE_LICENSE("GPL");
