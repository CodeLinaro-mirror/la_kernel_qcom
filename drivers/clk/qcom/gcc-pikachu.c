...
#include <dt-bindings/clock/qcom,pikachu-gcc.h>
...
static struct clk_vdd_class *gcc_pikachu_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_rcg2 gcc_pcie_0_aux_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = gcc_pikachu_regulators,
		.num_vdd_classes = ARRAY_SIZE(gcc_pikachu_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
...
static struct clk_branch gcc_usb30_prim_master_cloned_clk = {
	.halt_reg = 0x390a0,
	.halt_check = BRANCH_HALT_VOTED,
	.clkr = {
		.enable_reg = 0x390a0,
		.enable_mask = BIT(0),
		.hw.init = &(const struct clk_init_data) {
			.name = "gcc_usb30_prim_master_cloned_clk",
			.parent_hws = (const struct clk_hw*[]) {
				&gcc_usb30_prim_master_clk_src.clkr.hw,
			},
			.num_parents = 1,
...
static struct clk_regmap *gcc_pikachu_clocks[] = {
	...
	[GCC_USB30_PRIM_MASTER_CLONED_CLK] = &gcc_usb30_prim_master_cloned_clk.clkr,
...
static const struct qcom_reset_map gcc_pikachu_resets[] = {
	...
	[GCC_PDM_BCR] = { 0x33000 },
	[GCC_QUPV3_WRAPPER_0_BCR] = { 0x1e000 },
	...
	[GCC_VIDEO_XO_CLK_ARES] = { 0x3202c, 2 },
};
...
static const struct regmap_config gcc_pikachu_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct of_device_id gcc_pikachu_match_table[] = {
	{ .compatible = "qcom,pikachu-gcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_pikachu_match_table);

static int gcc_pikachu_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_pikachu_desc);
	if (IS_ERR(regmap))
...
static void gcc_pikachu_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_pikachu_desc);
}

static struct platform_driver gcc_pikachu_driver = {
	.probe = gcc_pikachu_probe,
	.driver = {
		.name = "gcc-pikachu",
		.of_match_table = gcc_pikachu_match_table,
		.sync_state = gcc_pikachu_sync_state,
	},
};

static int __init gcc_pikachu_init(void)
{
	return platform_driver_register(&gcc_pikachu_driver);
}
subsys_initcall(gcc_pikachu_init);

static void __exit gcc_pikachu_exit(void)
{
	platform_driver_unregister(&gcc_pikachu_driver);
}
module_exit(gcc_pikachu_exit);

MODULE_DESCRIPTION("QTI GCC PIKACHU Driver");
MODULE_LICENSE("GPL");
