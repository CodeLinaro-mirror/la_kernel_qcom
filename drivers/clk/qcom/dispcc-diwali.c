...
static struct clk_vdd_class *disp_cc_diwali_regulators[] = {
	&vdd_cx,
};
...
static struct clk_regmap *disp_cc_diwali_clocks[] = {
...
static const struct qcom_reset_map disp_cc_diwali_resets[] = {
	[DISP_CC_MDSS_CORE_BCR] = { 0x8000 },
	[DISP_CC_MDSS_CORE_INT2_BCR] = { 0xa000 },
	[DISP_CC_MDSS_RSCC_BCR] = { 0xc000 },
};

static const struct regmap_config disp_cc_diwali_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static struct qcom_cc_desc disp_cc_diwali_desc = {
	.config = &disp_cc_diwali_regmap_config,
	.clks = disp_cc_diwali_clocks,
	.num_clks = ARRAY_SIZE(disp_cc_diwali_clocks),
	.resets = disp_cc_diwali_resets,
	.num_resets = ARRAY_SIZE(disp_cc_diwali_resets),
	.clk_regulators = disp_cc_diwali_regulators,
	.num_clk_regulators = ARRAY_SIZE(disp_cc_diwali_regulators),
};

static const struct of_device_id disp_cc_diwali_match_table[] = {
	{ .compatible = "qcom,diwali-dispcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, disp_cc_diwali_match_table);

static int disp_cc_diwali_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &disp_cc_diwali_desc);
	if (IS_ERR(regmap))
		return PTR_ERR(regmap);

	ret = qcom_cc_runtime_init(pdev, &disp_cc_diwali_desc);
	if (ret)
		return ret;
	...
	ret = qcom_cc_really_probe(&pdev->dev, &disp_cc_diwali_desc, regmap);
	if (ret) {
...
static void disp_cc_diwali_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &disp_cc_diwali_desc);
}

static const struct dev_pm_ops disp_cc_diwali_pm_ops = {
	SET_RUNTIME_PM_OPS(qcom_cc_runtime_suspend, qcom_cc_runtime_resume, NULL)
	SET_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
				pm_runtime_force_resume)
};

static struct platform_driver disp_cc_diwali_driver = {
	.probe = disp_cc_diwali_probe,
	.driver = {
		.name = "disp_cc-diwali",
		.of_match_table = disp_cc_diwali_match_table,
		.sync_state = disp_cc_diwali_sync_state,
		.pm = &disp_cc_diwali_pm_ops,
	},
};

static int __init disp_cc_diwali_init(void)
{
	return platform_driver_register(&disp_cc_diwali_driver);
}
subsys_initcall(disp_cc_diwali_init);

static void __exit disp_cc_diwali_exit(void)
{
	platform_driver_unregister(&disp_cc_diwali_driver);
}
