...
# $DRIVERS. Said other way, in a provied SHA, non-existing files are omitted
...
function files()
{
  ...
  $DEBUG && {
    debug_off;
    $INTERACTIVE && read -rp "To continue press [enter] key ...";
  }
}
...
function is_merge()
{
  ...
  local -i parents_number=1;

  parents_number="$(
    commit_parents "$sha" | wc -l;
  )" || return 1;

  if [[ $parents_number -eq 1 ]]; then
    echo false; # regular commit
    return 0;
  elif [[ $parents_number -ge 2 ]]; then
...
function git-add-alternate()
{
  ...
  [[ -d "$base_repo" && -d "$target_objects" ]] || return 1;
